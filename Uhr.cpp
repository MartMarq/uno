...
void Uhr::drawWatchFace() {
  ...
  if (Welche == 1) {
    ...
    display.println(s1);
    display.drawBitmap(40, 160, xdurch, 20, 21, GxEPD_BLACK);
  ...
  } else if (Welche == 2) {
    {
      ...
      display.print("t - 6  ");
      display.println(s7);
      display.writeFastHLine(0, 175, 200, GxEPD_BLACK);
    ...
    int16_t xc, yc;
    uint16_t wc, hc;
    const uint8_t* UNO[] = { Xuno0, Xuno1, Xuno2, Xuno3, Xuno4, Xuno5, Xuno6, Xuno7, Xuno8, Xuno9 };

    xc = (stundeA * 100) + minuteA;

    for (n = 4; n > 0; n = n - 1) {
      yc = xc / (pow(10, n - 1));
      xc = xc - yc * (pow(10, n - 1));
      display.drawBitmap(200 - n * 50, 0, UNO[yc], 50, 78, GxEPD_BLACK);
    }
    ...
    for (n = 4; n > 0; n = n - 1) {
      yc = xc / (pow(10, n - 1));
      xc = xc - yc * (pow(10, n - 1));
      display.drawBitmap(200 - n * 50, 74, UNO[yc], 50, 78, GxEPD_BLACK);
    }
    xc = currentTime.Wday;
    if (xc == 1 || xc == 7) {
      display.drawBitmap(33, 150, Xunoplus4, 78, 50, GxEPD_BLACK);
    } else {
      display.drawBitmap(33, 150, Xunoplus, 78, 50, GxEPD_BLACK);
    }
    display.setFont(&FreeMonoBoldOblique22pt7b);
    display.getTextBounds(String(Protz), 100, 100, &xc, &yc, &wc, &hc);
    display.setCursor(150 - wc / 2, 200 - hc / 2);
